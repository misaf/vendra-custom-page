...
The `misaf/vendra-custom-page` package owns CMS custom pages and page categories and the Filament admin UI for custom pages and page categories.
...
- Keep API Platform schemas, resources, queries, routes, and server wiring in `misaf/vendra-custom-page-api`; do not add API transport code to this domain package.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets plus a tenant-agnostic expectation, e.g. `arch()->expect('Misaf\VendraCustomPage')->not->toUse('Misaf\VendraTenant')`.
